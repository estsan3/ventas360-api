...
Sin endpoints de escritura HTTP: el debe lo escribe **compras** y el haber **pagos**. El router solo consulta saldos y estado de cuenta. Idempotente por referencia; sin commit en el contrato.
...
`ContratoCxp` también expone `registrar_haber` y `saldo_proveedor`. El haber operativo lo escribe **pagos** (`POST /pagos`). Ver [pagos.md](pagos.md).
