...
`POST /ventas/pedidos`: valida cliente, arma líneas con `ContratoProductos` + `ContratoPrecios`, IVA y número de `ContratoParametros`, commit, evento `ventas.{tipo}.creado`. El remito exige `deposito_id`.

`PATCH .../estado` con remito + `estado=confirmado` redirige a `confirmar_remito`. Cualquier otra transición publica `ventas.{tipo}.estado_cambiado`.
