...
## Excepción: webhook n8n (IA)

`GET /api/v1/ai/webhook/resumen-dia` no usa JWT. Valida `X-Ventas360-Webhook-Secret` y fija el tenant con `X-Tenant-Slug` (`TenantsService.obtener_por_slug` + `usando_tenant`). Ver [ia.md](ia.md).
