...
sequenceDiagram
    ...
    Service->>BO: construir_acciones
    alt narrativa=false
        Note over Service: narrativa y modo_narrativa quedan vacíos
    else narrativa y anthropic_api_key y remito_parse_modo no mock
...
Query `narrativa=true` (default) en `GET /ai/resumen-dia` y en el webhook. Con `narrativa=false` no llama al LLM ni al mock.
