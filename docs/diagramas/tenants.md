...
El Host del request se lee con `hostname_desde_request` (`X-Forwarded-Host` / `X-Original-Host`, `Origin`, `Referer`, `Host`).
...
sequenceDiagram
    ...
    Cliente->>Router: GET /tenants/contexto + Host (o X-Forwarded-Host)
